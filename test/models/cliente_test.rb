...
class ClienteTest < ActiveSupport::TestCase
  test "cliente com nome e email válidos é válido" do
    cliente = Cliente.new(nome: "Maria Silva", email: "novo.cliente@example.com")
  ...
  test "é inválido sem nome" do
    ...
    assert cliente.errors.added?(:nome, :blank)
  end
  ...
  test "é inválido sem email" do
    ...
    assert_not cliente.valid?
    assert cliente.errors.added?(:email, :blank)
  end
  ...
  test "busca_por_nome retorna match exato" do
    resultado = Cliente.busca_por_nome("Maria")
    assert_equal 1, resultado.count
    assert_equal "Maria Silva", resultado.first.nome
  end

  test "busca_por_nome é case-insensitive" do
    resultado = Cliente.busca_por_nome("MARIA")
    assert_equal 1, resultado.count
  end

  test "busca_por_nome faz match parcial" do
    resultado = Cliente.busca_por_nome("Sil")
    assert_equal 1, resultado.count
    assert_equal "Maria Silva", resultado.first.nome
  end

  test "busca_por_nome retorna múltiplos matches" do
    Cliente.create!(nome: "Pedro Silva", email: "pedro@example.com")
    resultado = Cliente.busca_por_nome("Silva")
    assert_equal 2, resultado.count
    nomes = resultado.pluck(:nome).sort
    assert_equal ["Maria Silva", "Pedro Silva"], nomes
  end

  test "busca_por_nome sem match retorna relation vazia" do
    resultado = Cliente.busca_por_nome("inexistente")
    assert_equal 0, resultado.count
  end

  test "busca_por_nome trata '%' como caractere literal (não curinga)" do
    Cliente.create!(nome: "Promo 50%", email: "promo@example.com")
    # Buscar por "%" sozinho NÃO deve retornar tudo — apenas nomes com '%' literal
    resultado = Cliente.busca_por_nome("%")
    assert_equal 1, resultado.count
    assert_equal "Promo 50%", resultado.first.nome
  end

  test "busca_por_nome trata '_' como caractere literal (não curinga)" do
    ...
    resultado = Cliente.busca_por_nome("user_name")
    assert_equal 1, resultado.count
    assert_equal "user_name", resultado.first.nome
  end

  test "busca_por_nome trata backslash como caractere literal" do
    Cliente.create!(nome: 'foo\\bar', email: "fb@example.com")
    Cliente.create!(nome: 'foobar',   email: "b@example.com")
    resultado = Cliente.busca_por_nome('foo\\bar')
    assert_equal 1, resultado.count
    assert_equal 'foo\\bar', resultado.first.nome
  end

  test "busca_por_nome com string vazia retorna relation vazia" do
    # Sem o early-return, ILIKE '%%' casaria com todos os registros — bug.
    assert_equal 0, Cliente.busca_por_nome("").count
  end

  test "busca_por_nome com whitespace puro retorna relation vazia" do
    assert_equal 0, Cliente.busca_por_nome("   ").count
  end

  test "busca_por_nome com nil retorna relation vazia (não lança exceção)" do
    assert_nothing_raised do
      assert_equal 0, Cliente.busca_por_nome(nil).count
    end
  end

  test "busca_por_nome com acento bate apenas com forma acentuada (limitação documentada)" do
    ...
    assert_equal 1, Cliente.busca_por_nome("João").count
    assert_equal 0, Cliente.busca_por_nome("Joao").count
  end

  test "busca_por_nome retorna ActiveRecord::Relation (encadeável)" do
    relation = Cliente.busca_por_nome("Silva")
